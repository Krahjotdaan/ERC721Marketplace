...
contract ERC721FixedPriceMarketplace is IERC721Receiver, ReentrancyGuard {
    ...
    function listItemOnAuction(address _tokenAddress, uint256 _tokenId, uint256 _startPrice) external whenNotPaused {
        ...
        emit ListItemOnAuction(lastAuctionId, _tokenAddress, _tokenId, _startPrice);
    }

    function makeBid(uint256 _auctionId) external payable nonReentrant whenNotPaused {
    ...
    function withdrawToken(uint256 _auctionId) external {
        ...
        require(
            token.ownerOf(auction.tokenId) == address(this),
    ...
    function completeAuction(uint256 _auctionId) external nonReentrant whenNotPaused {
        ...
        if (auction.currentBidder == address(0)) {
            ...
            frozenEth -= auction.currentBid;
            token.transferFrom(address(this), auction.currentBidder, auction.tokenId);
        }
